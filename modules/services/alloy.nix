  ...
  renderers = rec {
    ...
    attrset =
      ...
      let
        ...
        argAttrs = lib.filterAttrs (
          name: v: (!builtins.isAttrs v) || (builtins.elem name not_blocks)
        ) attrs;

        blocks = lib.mapAttrsToList (name: value: ''
          ${name} {
            ${tab (attrset value)}
    ...
    namedVal =
      name: value:
      (lib.findFirst (v: v.name == name) { value = val; } (lib.attrsToList special)).value value;
