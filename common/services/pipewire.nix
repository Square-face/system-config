{ config, lib, ... }:
{
  ...
  config.services.pipewire.extraConfig.pipewire."92-low-latency" = lib.mkIf config.pipewire.lowLatency {
    "context.properties" = {
  ...
  config.services.pipewire.extraConfig.pipewire-pulse."92-low-latency" = lib.mkIf config.pipewire.lowLatency {
    "context.modules" = [
      {
